...
class mainVDSR(sub_main):
        ...
        if args.cuda:
            for iteration, batch in enumerate(test_data_loader):
                ...
                with torch.no_grad():
                    ...
                    utils.save_image(result_img,'test_result/VDSR/'+filename[0]+'_{}.png'.format(epoch))

        print("VDSR ==> Saved result at [test_result] .. ")
    ...
    def save_model(self, model, epoch):
        # TO-DO : 모델명!
        model_out_path = "VDSR_train_{}.pth".format(epoch)
        torch.save(model.state_dict(), './checkpoint/VDSR/' + model_out_path)
    ...
    def main(self):
        ...
        if self.args.run_type == 'train':
            ...
            model = VDSR.VDSR()
            print("VDSR ==> Setting optimizer .. [ ", self.args.optimizer, " ] , lr [ ", self.args.lr, " ] , Loss [ MSE ]")
            ...
            if os.path.exists(self.args.pre_model_dir):
                if not os.path.exists(self.args.dir_data_test_lr): print("VDSR ==> Fail [ Test model is not exists ]")
                else:
                    test_data_loader = loader.loader_test
            ...
            else : print("VDSR ==> Fail [ Pretrain model directory is not exists ]")
